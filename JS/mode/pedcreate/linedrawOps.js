...
class LineDrawOps {
	...
	updateLine() {
		this._RLineMethod(this._tmpLine, this._startPoint, this._endPoint);
	}
	...
	beginLineDraw() {
		...
		_this._tmpRect.on('mousemove', event => {
			if (_this._startPoint.x !== -1) {
				const mouseX = Math.floor(event.evt.clientX / grid_rezX) * grid_rezX,
		...
		_this._tmpRect.on('mouseup', event => {
	...
	drawCircles() {
		...
		const circleGroup = new Kinetic.Group({ x: 0, y: 0 });

		for (const perc_id in personDraw.used_ids) {
			const personIn = personDraw.used_ids[perc_id];

			if (personIn.family !== _this._family) {
				continue;
			}

			const gfx = personIn.gfx;

			const apos = gfx.getAbsolutePosition(),
				  rad = 15;

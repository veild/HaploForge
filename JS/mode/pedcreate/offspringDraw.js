...
class OffspringDraw extends LineDrawOps {
	...
	__drawNodes() {
		...
		const familyID = _this._family;

		const fam_gfx = uniqueGraphOps.getFam(familyID),
			  fam_group = fam_gfx.group,
			  edge_map = fam_gfx.edges,
			  node_map = fam_gfx.nodes;

		const nodeGroup = new Kinetic.Group();
		...
		for (const key in edge_map) {
			if (key[0] === 'm') {
				...
				const mateline_graphics = edge_map[key].graphics;

				// Sib_Anchor node is TEMPORARY. It is deleted upon ~offspringDraw()
				const node = Graphics.Shapes.addCircle('white', nodeSize / 2);
				...
				const startID = edge_map[key].start_join_id,
					  endID = edge_map[key].end_join_id;

				const startGraphics = node_map[startID].graphics,
	...
	__joinIDs() {
		const u_childline = edgeAccessor.childlineID(this.matelineID, this.childNodeID);

		const father_mother_ids = this.matelineID.split(':')[1].split('-'),
		...
		uniqueGraphOps.insertEdge(u_childline, this._family, new_line, this.matelineID, child.id, 2, false);

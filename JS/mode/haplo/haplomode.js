const HaploWindow = {
	_left: new Kinetic.Rect({
		//  side bar underlying markers
		x: 10,
		y: 95,
		height: 100,
		width: 65,
	...
	destroy: function stopHaplomode() {
		...
		HaploWindow._toggleBottom(false, () => {
			for (const fid in SelectionMode._ids) {
				for (const id in SelectionMode._ids[fid]) {
					const perc_gfx = uniqueGraphOps.getFam(fid).nodes[id].graphics;
					const old_pos = perc_gfx.main_layer_pos;
					const old_group = perc_gfx.main_layer_group;
	...
	init: function launchHaplomode() {
		...
		const line_data = findDOSinSelection(SelectionMode._ids_map);
	...
	_makeMainWindow(lines_nodes_to_render) {
		const res = mapLinesAndNodes(lines_nodes_to_render); // DOS.js

		const line_points = res.lp,
			  slot_array = res.sa;
	...
	_makeTop() {
		const render_group = DOS.group;
		const min_pos = DOS.min,
			  max_pos = DOS.max;
	...
	__showBottom(finishfunc = 0) {
		...
		kineticTween({
			...
			onFinish() {
				const scroll_area__ = new Kinetic.Group({
					draggable: true,
					dragBoundFunc(pos) {
						const group_loc = this.parent.getAbsolutePosition();

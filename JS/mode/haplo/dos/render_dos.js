const DOS = {
	...
	render(line_points, slot_array, finishfunc = 0) {
		//
		// Renders to DOS.group
		//
		const min_pos = { x: 9999, y: 9999 },
			  max_pos = { x: 0, y: 0 };
		...
		let render_counter = slot_array.length - 1;

		for (let fd = 0; fd < slot_array.length; fd++) {
			const fid_id = slot_array[fd][0].split('_'),
			...
			const gfx = uniqueGraphOps.getFam(fid).nodes[id].graphics;
		...
		for (let t = 0; t < tween_nodes.length; ) {
			tween_nodes[t++].play();
		}
...
function mapLinesAndNodes(line_map) {
	const slot_array = []; // index --> gfx

	function sortXHaplo(pos_y, id, fid) {
		const key = `${fid}_${id}`;

		for (let k = 0; k < slot_array.length; k++) {
	...
	const line_points = {};
	const end_point_nodes_drawn = {}; // Nodes with lines already attached (no multiple lines)

	function addLinePoint(fid, key, obj) {
		if (key in line_points[fid]) {
			const eski_obj = line_points[fid][key],
	...
	for (const fid in line_map) {
		...
		for (let g = 0; g < line_map[fid].length; g++) {
			//			console.log("gen=", g, line_map[fid][g])

			// ConnectEEs and connectERs...  FUCK THIS
			for (const sgroup in line_map[fid][g]) {
				//				console.log("SIB GROUP", sgroup)
				//start_y += drop_amount;

				const directline = line_map[fid][g][sgroup].directlines,
					  mateline = line_map[fid][g][sgroup].matelines;

				for (const mline in mateline) {
					const parents = mline.split('_'),
						  fath_id = parents[0],
						  moth_id = parents[1];
					...
					// Add mate line
					const consang = uniqueGraphOps.getFam(fid).edges[`m:${fath_id}-${moth_id}`].consangineous;

					addLinePoint(fid, fath_id, {
						to: moth_id,
						consang: consang,
						drop: null,
						text: null,
						lastgen: g == line_map[fid].length - 1
					});
					...
					if (isLastGen) {
						addLinePoint(fid, `${fath_id}_${moth_id}`, {
							to: sgroup,
							consang: false,
							drop: drop_amount,
							text: dos,
				...
				// Directline
				for (const dline in directline) {
					// Check if dline key is not already part of a mating, (just use that)
					// Easier to check here than upstream
					let found_existing_mateline = false;
					const linep_keys = Object.keys(line_points[fid]);

					for (let kk = 0; kk < linep_keys.length; kk++) {
						let key_ids = linep_keys[kk].split('_');

						if (key_ids.length != 2) key_ids = [ key_ids ];

						for (let ik = 0; ik < key_ids.length; ik++) {
							if (dline === key_ids[ik]) {
				...
				const sib_ids = sgroup.split('_');
				const sib_stepper = start_y + drop_amount; // + (back_step*(sib_ids.length -1));

				for (let s = 0; s < sib_ids.length; s++) {
...
function mapLines(line_points, haplo_group_lines) {
	...
	for (const fid in line_points) {
		for (const from in line_points[fid]) {
			const start_ids = from.split('_');

			const info = line_points[fid][from];

			let sib_anchor_pos = null;

			// Mateline or DOSline
			if (start_ids.length === 1) {
				const from_gfx_pos = uniqueGraphOps.getFam(fid).nodes[from].graphics.getPosition();
				...
				// DOS line -- mate
				const parent1_id = start_ids[0],
					  parent2_id = start_ids[1];

				const parent1_gfx = uniqueGraphOps.getFam(fid).nodes[parent1_id].graphics.getPosition(),
					  parent2_gfx = uniqueGraphOps.getFam(fid).nodes[parent2_id].graphics.getPosition();

				const mid_point_pos = { x: (parent1_gfx.x + parent2_gfx.x) / 2, y: parent1_gfx.y };
			...
			if (sib_anchor_pos !== null) {
				const sib_ids = info.to.split('_');

				let min_x_of_sibgroup = 99999999999,
					y_pos_of_any_sibline = 999999999;

				for (let s = 0; s < sib_ids.length; s++) {
					const sib_id = sib_ids[s],
					...
					else sibline = Graphics.Lines.addRLine_simple(sib_anchor_pos, sib_gfx_pos, false);
				...
				if (info.text > 1) {
					haplo_group_lines.add(
						createDOSCircle((sib_anchor_pos.x + min_x_of_sibgroup) / 2, y_pos_of_any_sibline, info.text)
					);
				}
			}
		}
...
var createDOSCircle = (xx, yy, dos_text) => {

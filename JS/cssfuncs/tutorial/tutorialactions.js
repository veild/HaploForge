class TutorialActions {
	...
	backwardVisible(vis) {
		TutorialActions.__buttonVisibility(this.back, vis);
	}
	forwardVisible(vis) {
		TutorialActions.__buttonVisibility(this.forw, vis);
	}
	...
	forwardPage() {
		...
		const len = this._pages.length, next = this._currentpage + 1;
	...
	__hidepage(pageno) {
		this.___setpage(pageno, false);
	}
	__showpage(pageno) {
		this.___setpage(pageno, true);
	}

	__fadeTransition(obj, num_levels, start, interval, callback) {
		const op_step = 1 / num_levels, op_curr = start,
			  op_mod = start === 0 ? 1 : -1;

		const wait_step = interval / num_levels;
	...
	___setpage(pageno, visible, interval = 1000) {
		...
		if (visible) {
			setTimeout(() => {
				...
				console.log(plot);
				if (plot.enterAction !== null) {
					plot.enterAction();
				}
				page.style.display = 'block';
			}, TutorialActions.__wait_amount);
		} else {
			this.__fadeTransition(page, 4, 1, 200, () => {
				page.style.opacity = 0;
				if (plot.exitAction !== null) {
					plot.exitAction();
				}

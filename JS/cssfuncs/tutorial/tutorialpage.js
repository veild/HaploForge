class TutorialPage {
	constructor(object) {
		...
		if (object.type === 'video') {
			const trans = object.media.text[0];
	...
	__makePage() {
		const divmain = document.createElement('div'),
              ...
              // top text
              toptext = document.createElement('h6'),
		...
		if (this.__media !== undefined) {
			...
			if (this.__type === 'video') {
				const vt = new VideoTranscript(that, this.__media.src, this.__media.text);
				med = vt.getVideo();
				...
				if (this.__action.enter === null) {
					...
					const enter = this.__action.enter;
					this.__action.enter = () => {
	...
	modifyTop(text) {
		this.__transitionText(this.text.top, text, 500);
	}
	modifyBot(text) {
		this.__transitionText(this.text.bot, text, 500);
	}
}

const AstarHandler = {
	...
	pushAll(from, to) {
		...
		for (let j = 0; j < from.color_group.length; j++) {
			if (to.color_group.indexOf(from.color_group[j]) == -1) to.color_group.push(from.color_group[j]);
		}
	},

	/** Reset search if incompatible genotypes **/
	resetCheck(p1, p2) {
		const reset = p1.color_group.length === 0 || p2.color_group.length === 0;
		if (reset) {
			p1.color_group = [];
			p2.color_group = [];
		}
	...
	*/
	child2parent_link(
		...
		moth,
		fath,
		fam  // fam only needed for consang check.
	) {
		const pers_hp = pers.haplo_data,
			  moth_hp = moth.haplo_data,
			  fath_hp = fath.haplo_data,
			  gender = pers.gender;

		console.assert(
			pers_hp[0].data_array.length === moth_hp[0].data_array.length &&
				pers_hp[0].data_array.length === fath_hp[0].data_array.length &&
				pers_hp[1].data_array.length === moth_hp[1].data_array.length &&
				pers_hp[1].data_array.length === fath_hp[1].data_array.length,
				`Allele lengths dont match for moth,fath,chil:${moth.id} ${fath.id} ${pers.id}`
		);
		...
		while (++tmp_i < pers_hp[0].data_array.length) {
			...
			const f0_ht = fath_hp[0].data_array[tmp_i],
				// Y allele, potential pitfall here -- input rows either follow a specific XY order, or need to do postproc.
				f1_ht = fath_hp[1].data_array[tmp_i],
				m0_ht = moth_hp[0].data_array[tmp_i],
				m1_ht = moth_hp[1].data_array[tmp_i];

			const f0_pr = fath_hp[0].pter_array[tmp_i],
			...
			const c0_ht = pers_hp[0].data_array[tmp_i], // X allele
				  c1_ht = pers_hp[1].data_array[tmp_i]; // Y allele

			const c0_pr = pers_hp[0].pter_array[tmp_i],
			...
			/* -- Sex-linked and male scenario:
			  ...
			  female: 0{2,3} = 02 03
			    male: 1{2,3} = 12 13
			 */
			if (HaploPedProps.xlinked && gender === PED.MALE) {
				// Sexlinked and male
		...
		*/
		const maternal_exclusion = moth_hp[0].unique_groups.concat(moth_hp[1].unique_groups),
			  paternal_exclusion = fath_hp[0].unique_groups.concat(fath_hp[1].unique_groups);

		// Check for consanginuity
		const key = `m:${fath.id}-${moth.id}`;
		const consang_check = uniqueGraphOps.getFam(fam).edges[key].consangineous;
		...
		if (consang_check) {
			...
			for (let ov1 = 0; ov1 < maternal_exclusion.length; ov1++)
				for (let ov2 = 0; ov2 < paternal_exclusion.length; ov2++)
					if (maternal_exclusion[ov1] === paternal_exclusion[ov2]) toKick.push(maternal_exclusion[ov1]);

			if (toKick.length !== 0) {
				for (const val of toKick) {
					const maternal_index = maternal_exclusion.indexOf(val),
		...
		let res0 = a_star_bestfirst(pers_hp[0].pter_array, maternal_exclusion),
			res1;
		...
		if (res1 === null) {
			...
			if (res0 === null || res1 === null) throw new Error("Skipper's Log: It's been days...");
		}

		const unique0 = res0.filter((item, i, ar) => {
			return item !== FounderColor.zero_color_grp && ar.indexOf(item) === i;
		});
		const unique1 = res1.filter((item, i, ar) => {
			return item !== FounderColor.zero_color_grp && ar.indexOf(item) === i;
		});
